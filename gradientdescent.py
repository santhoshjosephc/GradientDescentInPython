...
#Alpha is the size of the baby step that we will take for each of the variables (the predictors) (also called the learning rate). In this particular example, we only have one variable.
...
#a positive error and a negative error will cancel out each other which is not great. It gives a false sense that the error is lower than what it actually is. 
...
for j in range(10000):
    ...
    #and second one by X[i]? Answer: We are finding the derivative in order to minimize the function. Please read up on differntial calculus
    # if you dont understand what this step does and why it does that. It is too much to explain here. 
    for i in range(m):
